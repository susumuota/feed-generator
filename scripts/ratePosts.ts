...
const updatePost = async (uri: string, metric: string, rating: number, explanation: string) => (
  createDb(process.env.FEEDGEN_SQLITE_LOCATION ?? ':memory:')
    .updateTable('post')
...
const insertLlmUsage = async (uri: string, cid: string, promptTokens: number, completionTokens: number, totalTokens: number) => (
  createDb(process.env.FEEDGEN_SQLITE_LOCATION ?? ':memory:')
    .insertInto('llm_usage')
    .values({ uri, cid, indexedAt: new Date().toISOString(), promptTokens, completionTokens, totalTokens })
    .onConflict((oc) => oc.doNothing())
    .execute()
)

const createChatCompletionParams = (target: string, feature: string, text: string): CreateChatCompletionRequest => ({
  model: "gpt-3.5-turbo",
  messages: [
    {
      role: "system",
    ...
    {
      role: "user", content: `Rate this tweet: \n\n\`\`\`\n${text}\n\`\`\``,
    },
  ],
  functions: [
    {
